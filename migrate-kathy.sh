#!/bin/bash
# ─────────────────────────────────────────────────────────────
# Migrate Kathy's data from kathy-catalog-db to artie-kathy-db
# ─────────────────────────────────────────────────────────────

set -e

SRC="kathy-catalog-db"
DST="artie-kathy-db"
...
const artworks = loadResults('/tmp/kathy_artworks.json');
const genres = loadResults('/tmp/kathy_genres.json');
const artworkGenres = loadResults('/tmp/kathy_artwork_genres.json');
...
npx wrangler d1 execute $DST --remote --file=/tmp/kathy_import.sql

echo ""
echo "✅ Migration complete!"
npx wrangler d1 execute $DST --remote --command="SELECT COUNT(*) as artworks FROM artworks;"
