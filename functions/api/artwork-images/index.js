...
export async function onRequestPost({ env, request, data }) {
  ...
  const { artwork_id, image_key, caption, sort_order = 0, background_color = null } = await request.json()
  if (!artwork_id || !image_key) return Response.json({ error: 'artwork_id and image_key required' }, { status: 400 })
  const res = await env.DB.prepare(
    'INSERT INTO artwork_images (artwork_id, image_key, caption, sort_order, background_color) VALUES (?,?,?,?,?)'
  ).bind(artwork_id, image_key, caption || null, sort_order, background_color || null).run()
